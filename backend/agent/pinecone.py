...
def create_pinecone_vector_store(file, chunks, chunk_strategy):
    ...
    # List all namespaces in the index before creating new one
    try:
        stats = index.describe_index_stats()
        logger.info(f"Available namespaces before creation: {stats.namespaces.keys() if hasattr(stats, 'namespaces') else 'No namespaces found'}")
    except Exception as e:
    ...
    # List all namespaces in the index after creation
    try:
        stats = index.describe_index_stats()
        logger.info(f"Available namespaces after creation: {stats.namespaces.keys() if hasattr(stats, 'namespaces') else 'No namespaces found'}")
    except Exception as e:
        logger.error(f"Error getting index stats: {str(e)}")
...
def query_pinecone(
    ...
    try:
        ...
        try:
            stats = index.describe_index_stats()
            logger.info(f"Available namespaces: {stats.namespaces.keys() if hasattr(stats, 'namespaces') else 'No namespaces found'}")
        except Exception as e:
            logger.error(f"Error getting index stats: {str(e)}")
